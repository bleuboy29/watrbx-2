...
# СТВОРЮЄМО ФАЙЛ .ENV З ПАРАМЕТРАМИ SSL ДЛЯ САЙТУ
RUN echo "DB_HOST=\${DB_HOST}" > /var/www/html/.env && \
    ...
    echo "MYSQL_ATTR_SSL_CA=" >> /var/www/html/.env && \
    echo "DB_SSL=true" >> /var/www/html/.env

# Створюємо конфіг Phinx з підтримкою SSL режиму
